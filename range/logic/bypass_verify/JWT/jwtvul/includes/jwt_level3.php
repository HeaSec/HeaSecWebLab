<?php
/**
 * HeaSec天积安全团队 - JWT基础漏洞靶场 - 第三关JWT逻辑
 * 版本: v1.0.0
 * 创建日期: 2026-03-03
...
{
    /** @var string 密钥文件路径 */
    private static $keyFile = __DIR__ . '/keys/level3_secret.txt';
     ...
     * @param int|null $customExp 自定义过期时间（用于生成过期的测试Token）
     * @return string JWT Token
     */
    public static function encode($username, $role, $customExp = null)
    {
        ...
        $payload = [
            ...
            'role' => $role,
            'iat' => time(),
            'exp' => $customExp !== null ? $customExp : (time() + 86400) // 24小时有效期或自定义
        ];
