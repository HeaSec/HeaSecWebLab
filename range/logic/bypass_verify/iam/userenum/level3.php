...
// 设置响应头
header('X-HeavenlySecret: HeaSec UserEnum Range v1.0.0 - Level 3');
...
{
    // 检查测试账号是否存在
    $stmt = $pdo->prepare("SELECT id FROM heasec_userenum_users WHERE username = '13866668888' AND level = 3 LIMIT 1");
    $stmt->execute();
    if (!$stmt->fetch()) {
        // 创建测试账号
        $stmt = $pdo->prepare("INSERT INTO heasec_userenum_users (username, password, level, created_at) VALUES (?, ?, 3, NOW())");
        $stmt->execute(['13866668888', '123456']);
    }

    // 检查目标账号是否存在（1100591XXXX）
    $stmt = $pdo->prepare("SELECT username FROM heasec_userenum_users WHERE username LIKE '1100591%' AND level = 3 LIMIT 1");
    $stmt->execute();
    if (!$stmt->fetch()) {
        // 生成目标账号
