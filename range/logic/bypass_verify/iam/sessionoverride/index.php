...
// 当前关卡
$currentLevel = 1;
$levelTitle = '第一关：验证码与目标账号不绑定';
$nextPage = 'sessionoverride2.php';
$nextBtnText = '下一关';
...
function getOrCreateAdminUser($level, $pdo) {
    ...
    $phone = '11066668888'; // admin账号的手机号（使用保留号段，短信模拟器无法查看）
...
// 检查登录状态
$isLoggedIn = isset($_SESSION['sessionoverride_level1_logged_in']) && $_SESSION['sessionoverride_level1_logged_in'] === true;
$currentUser = isset($_SESSION['sessionoverride_level1_user']) ? $_SESSION['sessionoverride_level1_user'] : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    if ($user && $user['password'] === $password) {
        $_SESSION['sessionoverride_level1_logged_in'] = true;
        $_SESSION['sessionoverride_level1_user'] = $user;
        $isLoggedIn = true;
        $currentUser = $user;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'logout') {
    unset($_SESSION['sessionoverride_level1_logged_in']);
    unset($_SESSION['sessionoverride_level1_user']);
    $isLoggedIn = false;
    $currentUser = null;
}
    ...
    <div class="tech-card">
        ...
        <div class="tech-card-body">
            ...
            <div class="alert-dev-tip">
                ...
                <span>开发人员ps：验证码发到指定的手机上了，安全得很！</span>
            </div>
        ...
        <div class="heasec-modal-body">
            <form id="forgotPasswordForm">
                ...
                <div class="form-group">
                    ...
                    <input type="password" id="reset_password" name="password" class="tech-input" placeholder="请输入新密码" autocomplete="off">
                </div>
                <div class="form-group">
                    <label for="reset_confirm_password" class="form-label">
                        <i class="fa fa-lock"></i> 确认密码
                    </label>
                    <input type="password" id="reset_confirm_password" name="confirm_password" class="tech-input" placeholder="请再次输入新密码" autocomplete="off">
                </div>
                <div class="form-group">
                    <label for="reset_captcha" class="form-label">
                        <i class="fa fa-shield"></i> 验证码
                    </label>
                    <div class="captcha-row">
                        <input type="text" id="reset_captcha" name="captcha" class="tech-input" placeholder="请输入验证码" autocomplete="off" maxlength="6">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        initSessionOverride(<?php echo $currentLevel; ?>);
    });
</script>
