...
// 设置响应头
header('X-HeavenlySecret: HeaSec BruteEnc Range v1.0.0 - Level 2');
...
{
    // 检查admin用户是否存在（第二关）
    $stmt = $pdo->prepare("SELECT username, password FROM heasec_bruteenc_users WHERE username = 'admin' AND level = 2 LIMIT 1");
    $stmt->execute();
    ...
    $stmt = $pdo->prepare("INSERT INTO heasec_bruteenc_users (username, password, level, created_at) VALUES (?, ?, 2, NOW())");
