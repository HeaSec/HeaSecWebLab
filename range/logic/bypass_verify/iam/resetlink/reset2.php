<?php
/**
 * HeaSec天积安全团队 - 密码重置凭证可猜测靶场 - 第二关密码重置页面
 * 版本: v1.0.0
 * 创建日期: 2026-01-22
...
if ($token) {
    ...
    //验证token是否等于用户名的MD5值
    $stmt = $pdo->prepare("SELECT * FROM heasec_resetlink_users WHERE level = 2 AND MD5(username) = ?");
    $stmt->execute([$token]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reset_password' && $valid) {
    ...
    if (empty($newPassword)) {
        ...
        $pdo = HeaSec_Database::getConnection('heasec_logic');
        $stmt = $pdo->prepare("UPDATE heasec_resetlink_users SET password = ? WHERE level = 2 AND username = ?");
        ...
        <div class="tech-card-body">
            <?php if ($valid && $targetUser): ?>
                ...
                <div class="form-actions">
                    <a href="resetlink2.php" class="tech-btn tech-btn-primary">
