...
// 当前关卡
$currentLevel = 2;
$levelTitle = '第二关：密码重置凭证可猜测（MD5值）';
$nextPage = 'resetlink3.php';
$nextBtnText = '下一关';
...
// 检查登录状态
$isLoggedIn = isset($_SESSION['resetlink_level2_logged_in']) && $_SESSION['resetlink_level2_logged_in'] === true;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    if ($user && $user['password'] === $password) {
        $_SESSION['resetlink_level2_logged_in'] = true;
        $_SESSION['resetlink_level2_user'] = $user;
        $isLoggedIn = true;
        $currentUser = $user;
        ...
        if ($user['friend_added'] == 1 && !empty($user['friend_username'])) {
            ...
            if ($friendUser) {
                $_SESSION['resetlink_level2_user']['friend_info'] = $friendUser;
                // 同步更新$currentUser变量
                $currentUser = $_SESSION['resetlink_level2_user'];
            }
        }
    } else {
        $loginError = '账号或密码错误';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'logout') {
    unset($_SESSION['resetlink_level2_logged_in']);
    unset($_SESSION['resetlink_level2_user']);
    $isLoggedIn = false;
    $currentUser = null;
}
        ...
        <div class="tech-card-body">
            ...
            <div class="alert alert-warning">
                ...
                <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(245, 158, 11, 0.2);">
                    <small style="color: #f59e0b;">开发人员ps：他们说不能用明文，唔，我处理了一下，这下你们看不懂了吧</small>
                </div>
            </div>
