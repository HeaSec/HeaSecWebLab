<?php
/**
 * HeaSec天积安全团队 - 密码重置凭证可猜测靶场 - 添加好友API（第三关）
 * 版本: v1.0.0
 * 创建日期: 2026-01-22
...
// 验证登录状态
if (!isset($_SESSION['resetlink_level3_logged_in']) || $_SESSION['resetlink_level3_logged_in'] !== true) {
    echo json_encode([
        'success' => false,
        'message' => '请先登录'
    ]);
...
try {
    ...
    if ($action === 'search') {
        // 查找好友账号
        $stmt = $pdo->prepare("SELECT username, user_id, phone FROM heasec_resetlink_users WHERE level = 3 AND username = ?");
        ...
        // 检查是否已添加好友
        $stmt = $pdo->prepare("SELECT friend_added FROM heasec_resetlink_users WHERE level = 3 AND id = ?");
    ...
    if ($action === 'add') {
        ...
        // 更新用户记录，标记已添加好友
        $stmt = $pdo->prepare("UPDATE heasec_resetlink_users SET friend_added = 1, friend_username = ?, updated_at = CURRENT_TIMESTAMP WHERE level = 3 AND id = ?");
        $stmt->execute([$friendUsername, $currentUser['id']]);

        // 更新session
        $_SESSION['resetlink_level3_user']['friend_added'] = 1;
        $_SESSION['resetlink_level3_user']['friend_username'] = $friendUsername;
