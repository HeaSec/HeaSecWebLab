<?php
/**
 * HeaSec天积安全团队 - 密码重置凭证可猜测靶场 - 第一关发送重置链接API
 * 版本: v1.0.0
 * 创建日期: 2026-01-22
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM heasec_resetlink_users WHERE level = 1 AND username = ?");
    ...
    $result = HeaSec_SmsSender::send($user['phone'], $message, 'resetlink_level1');
