...
// 当前关卡配置
$currentLevel = 2;
$levelTitle = '第二关：本系统调试好了才上线的';
$taskHint = '登录管理员账号获取通关密码，提示：找找看是怎么调试的呢？';
$nextPage = 'level3.php?username=test&url=';
$nextBtnText = '下一关';
...
// 确保管理员活跃会话存在（首次访问时自动创建）
// 注意：ensureAdminSession内部会打开/关闭admin会话，会污染$_SESSION，
// 因此调用后必须重新初始化用户自己的会话以恢复正确的$_SESSION数据
    ...
    <div class="tech-card">
        ...
        <div class="tech-card-body">
            ...
            <form id="loginForm" class="tech-form" <?php echo $isLoggedIn ? 'style="display:none;"' : ''; ?>>
                ...
                <!-- 调试工具按钮（已禁用） -->
                <!-- <button class="tech-btn tech-btn-info" onclick="window.open('api/level2/debug-sessions.php')">调试工具</button> -->
            ...
            <div id="userInfoArea" style="<?php echo $isLoggedIn ? 'display:block;' : 'display:none;'; ?>">
                <div class="user-info">
                    ...
                    <div class="info-row">
                        <span class="info-label"><i class="fa fa-user-secret"></i> 角色：</span>
                        <span class="info-value" id="displayRole"><?php echo $isLoggedIn ? ($userData['role'] === 'admin' ? '管理员' : '普通用户') : ''; ?></span>
                    </div>
                </div>
