<?php
/**
 * HeaSec天积安全团队 - 三方支付漏洞靶场 - 第二关天积宝支付接口
 * 版本: v1.0.0
 * 创建日期: 2026-03-19
...
try {
    ...
    // 生成签名并回调通知电商
    $timestamp = time();
    $sign = generateCallbackSignV2($orderId, 'success', strval($amount), strval($timestamp));
    $callbackData = [
        'order_id' => $orderId,
        'status' => 'success',
        'amount' => $amount,
    ...
    $callbackUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http')
        . '://' . $_SERVER['HTTP_HOST']
        . dirname(dirname($_SERVER['PHP_SELF'])) . '/level2/callback.php';
