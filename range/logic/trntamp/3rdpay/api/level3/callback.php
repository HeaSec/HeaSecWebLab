<?php
/**
 * HeaSec天积安全团队 - 三方支付漏洞靶场 - 第三关支付回调接口
 * 版本: v1.0.0
 * 创建日期: 2026-03-19
...
// 验证签名
if (!verifyCallbackSignV2($orderId, $status, $amount, $timestamp, $sign, $discount)) {
...
try {
    ...
    // 计算实际支付金额
    $paidAmount = floatval($amount) - floatval($discount);

    // 更新订单状态
    updateOrderPaid($orderId, $paidAmount, $pdo);
