...
try {
    // 检查登录状态
    if (!isset($_SESSION['vertbp_level2_logged_in']) || $_SESSION['vertbp_level2_logged_in'] !== true) {
        throw new Exception('请先登录');
    }

    // 根据Cookie中的状态判断是否允许访问
    $isAdmin = isset($_COOKIE['vertbp_level2_is_admin']) && $_COOKIE['vertbp_level2_is_admin'] === '1';

    if (!$isAdmin) {
        throw new Exception('权限不足，仅管理员可访问');
    }
    ...
    // 获取admin用户的通关密码
    $stmt = $pdo->prepare("SELECT passcode FROM heasec_vertbp_users WHERE level = 2 AND role = 'admin'");
    $stmt->execute();
