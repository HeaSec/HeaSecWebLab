...
// 设置响应头
header('X-HeavenlySecret: HeaSec 文件上传前端JS绕过 Range v1.0.0');
header('Content-Type: text/html; charset=utf-8');

// 设置页面变量
$pageTitle = '文件上传前端JS绕过靶场';
$rangeName = '文件上传前端JS绕过';
$showVersion = false;
...
// 获取已上传的文件列表（过滤隐藏文件和系统文件）
$uploadedFiles = [];
if (file_exists($uploadDir)) {
    $files = scandir($uploadDir);
    foreach ($files as $file) {
        // 过滤 . 和 .. 以及以点开头的隐藏文件，以及 secret.php
        if ($file !== '.' && $file !== '..' && strpos($file, '.') !== 0 && $file !== 'secret.php') {
            $uploadedFiles[] = [
                'name' => $file,
                'path' => 'images/' . $file,
