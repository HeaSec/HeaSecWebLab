<?php
/**
 * HeaSec天积安全团队 - XSS HTML内容过滤靶场 - 通关API
 * 版本: v1.0.0
 * 创建日期: 2025-12-31
...
// 设置响应头
header('X-HeavenlySecret: HeaSec XSS通关API v1.0.0');
...
try {
    ...
    if ($level === 1 || $level === 2) {
        HeaSec_UpdateLearningStatus('xss_content_filter', '学习中');
    } elseif ($level === 3) {
        HeaSec_UpdateLearningStatus('xss_content_filter', '已掌握');
    }
