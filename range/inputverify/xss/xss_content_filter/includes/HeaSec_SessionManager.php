...
class HeaSec_SessionManager {
    ...
    private static function loadProgress() {
        try {
            $stmt = self::$db->prepare(
                "SELECT * FROM heasec_xss_content_filter_progress WHERE session_id = ? LIMIT 1"
            );
    ...
    private static function createProgress() {
        try {
            $stmt = self::$db->prepare(
                "INSERT INTO heasec_xss_content_filter_progress
    ...
    public static function completeLevel($level) {
        ...
        try {
            ...
            // 更新数据库
            $stmt = self::$db->prepare(
                "UPDATE heasec_xss_content_filter_progress
    ...
    private static function recordAchievement($level) {
        try {
            ...
            // 插入或更新成就记录
            $stmt = self::$db->prepare(
                "INSERT INTO heasec_xss_content_filter_records
    ...
    public static function getStarCount() {
        ...
        try {
            $stmt = self::$db->query(
                "SELECT COUNT(*) as count FROM heasec_xss_content_filter_records"
            );
    ...
    public static function resetProgress() {
        ...
        try {
            // 重置进度表
            $stmt = self::$db->prepare(
                "DELETE FROM heasec_xss_content_filter_progress WHERE session_id = ?"
            );
            $stmt->execute([self::$sessionId]);

            // 重置成就表
            $stmt = self::$db->prepare(
                "DELETE FROM heasec_xss_content_filter_records"
            );
            $stmt->execute();
    ...
    public static function getLevelDescription($level) {
        $descriptions = [
            1 => '大小写绕过 - HTML标签名对大小写不敏感',
            2 => '双写绕过 - 单次替换可以被关键词重组绕过',
            3 => '注释分割绕过 - 浏览器会忽略HTML注释'
        ];
