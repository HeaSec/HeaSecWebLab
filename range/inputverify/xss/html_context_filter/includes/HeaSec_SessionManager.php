<?php
/**
 * HeaSec HTML上下文XSS过滤靶场会话管理类
 * 版本: v1.0.0
 * 创建日期: 2026-01-14
 * 团队: 天积安全 (HeavenlySecret)
 *
 * 说明: 管理HTML上下文XSS过滤绕过靶场的用户会话和关卡进度
 */
...
class HeaSec_SessionManager {
    ...
    private static function loadProgress() {
        try {
            $stmt = self::$db->prepare(
                "SELECT * FROM heasec_html_context_filter_progress WHERE session_id = ? LIMIT 1"
            );
    ...
    private static function createProgress() {
        try {
            $stmt = self::$db->prepare(
                "INSERT INTO heasec_html_context_filter_progress
    ...
    public static function completeLevel($level) {
        ...
        try {
            ...
            // 更新数据库
            $stmt = self::$db->prepare(
                "UPDATE heasec_html_context_filter_progress
    ...
    private static function recordAchievement($level) {
        try {
            ...
            // 插入或更新成就记录
            $stmt = self::$db->prepare(
                "INSERT INTO heasec_html_context_filter_records
    ...
    public static function getStarCount() {
        ...
        try {
            $stmt = self::$db->query(
                "SELECT COUNT(*) as count FROM heasec_html_context_filter_records"
            );
    ...
    public static function resetProgress() {
        ...
        try {
            // 重置进度表
            $stmt = self::$db->prepare(
                "DELETE FROM heasec_html_context_filter_progress WHERE session_id = ?"
            );
            $stmt->execute([self::$sessionId]);

            // 重置成就表
            self::$db->query(
                "DELETE FROM heasec_html_context_filter_records"
            );
    ...
    public static function getLevelDescription($level) {
        $descriptions = [
            1 => '无引号属性注入 - 利用事件处理器绕过字符过滤',
            2 => '隐蔽属性注入 - 绕过关键字过滤',
            3 => 'HTTP头注入 - 利用JavaScript伪协议'
        ];
