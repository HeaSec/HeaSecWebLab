<?php
/**
 * HeaSec天积安全团队 - HTML上下文XSS过滤绕过靶场 - 通关API
 * 版本: v1.0.0
 * 创建日期: 2026-01-14
...
// 设置响应头
header('X-HeavenlySecret: HeaSec HTML上下文XSS通关API v1.0.0');
...
try {
    ...
    if ($level === 1 || $level === 2) {
        HeaSec_UpdateLearningStatus('html_context_filter', '学习中');
    } elseif ($level === 3) {
        HeaSec_UpdateLearningStatus('html_context_filter', '已掌握');
    }
