<?php
/**
 * HeaSec天积安全团队 - HTML上下文XSS过滤绕过靶场 - 第二关
 * 版本: v1.0.0
 * 创建日期: 2026-01-14
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['xss_code'])) {
        ...
        // 第二关：应用过滤规则
        $filteredCode = HeaSec_HTMLContextFilter::filterLevel2($inputCode);
    }
...
<script>
    (function () {
        ...
        window.alert = function (message) {
            ...
            console.log('[HeaSec XSS] 调用堆栈原始内容:', stack);
            ...
            if (stack.indexOf('UtilityScript.evaluate') !== -1 ||
                ...
                console.log('[HeaSec XSS] 检测到自动化工具执行特征');
            }
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (line.match(/at\s+(<anonymous>:\d+|VM\d+:\d+)/)) {
                            ...
                            if (line.indexOf('level2.php') === -1 &&
                                ...
                                line.indexOf('heasec') === -1) {
                                isDirectConsoleCall = true;
                                console.log('[HeaSec XSS] 检测到控制台调用特征:', line.trim());
                                break;
                            }
                        }
                    }
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    if (line.indexOf('eval') !== -1 && line.match(/<anonymous>:\d+/)) {
                        ...
                        if (match && match[1] === '1') {
                            // <anonymous>:1 通常是控制台
                            isDirectConsoleCall = true;
                            console.log('[HeaSec XSS] 检测到eval在<anonymous>:1执行');
                            break;
                        }
                    }
                }
            ...
            console.log('[HeaSec XSS] isDirectConsoleCall:', isDirectConsoleCall);
                        ...
                        <div class="search-container">
                            <div class="search-input-wrapper">
                                <textarea id="xss_code" name="xss_code" class="search-input auto-resize-textarea"
                                     onmouseover=alert(heasec)" rows="2" required
