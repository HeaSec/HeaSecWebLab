<?php
/**
 * HeaSec天积安全团队 - SQL特殊注入场景靶场 - 第二关登录接口
 * 版本: v1.0.0
 * 功能: 用户登录验证（参数化查询，安全）
...
try {
    // 登录查询使用参数化查询（安全，非注入点）
    $stmt = $pdo->prepare("SELECT id, username, role FROM heasec_specsi_accounts WHERE username = ? AND password = ?");
