-- ============================================================
-- HeaSec天积安全团队 - 报错注入靶场 - 数据库初始化脚本
-- 版本: v1.0.0
-- 数据库: heasec_sqli（与SQL盲注靶场共享）
-- 表名前缀: heasec_errsi_
...
-- 服务资产表（靶场业务数据）
-- -----------------------------------------------------------
DROP TABLE IF EXISTS heasec_errsi_services;
CREATE TABLE IF NOT EXISTS `heasec_errsi_services` (
...
INSERT INTO `heasec_errsi_services` (`service_name`, `service_type`, `status`, `port`, `description`) VALUES
...
-- 成就记录表（全局共享模式，无会话依赖）
-- -----------------------------------------------------------
DROP TABLE IF EXISTS heasec_errsi_achievements;
CREATE TABLE IF NOT EXISTS `heasec_errsi_achievements` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
